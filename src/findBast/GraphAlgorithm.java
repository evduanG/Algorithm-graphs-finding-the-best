...
class DocText extends TextArea {
	...
	final String done 		= new String("Algorithm has finished, " +"follow orange arrows from startnode to any node "+
	...
	final String some 		= new String("Algorithm has finished, " +	  "follow orange arrows from startnode to any node "+
	  "to get\nthe shortest path to " +	  "the node. The length of the path is written in the node.\n" +
	...
	final String doc = info + drawnodes + rmvnodes + mvnodes + startnode + drawarrows + weight + rmvarrows + 
...
class GraphCanvas extends Canvas implements Runnable {
	...
	public void paint(Graphics g) {
		...
		if (step==1)
			...
			showstring="Step " + step + ": Red arrows point to nodes reachable from " + "nodes that already have a final distance." + "\nThe distance to: ";
